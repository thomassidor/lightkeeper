...
| 3 | **done** | main | All 7 tasks. 590 tests. `lib/devices/` is a two-file split, not the single base class the plan named: `require('homey')` resolves only on a Homey, so anything inside a `class extends Homey.Device` cannot be imported by a test. `DeviceLifecycle` takes its host as an argument and carries every ordering rule; `LightkeeperDevice` is the thin SDK shell. See DEVIATIONS.md. **3.1 needs the init/pause/rename/repair/delete hardware pass across all three device types** — the loader indirection is the risk. |
| 4 | **done** | main | All 5 tasks. 629 tests. Overlap is a WEEKLY question, not a daily one — `entriesOverlap` lays each entry out as one arc per start day on a 10 080-minute circle, which is the only shape that catches Fri 23:30+2h against Sat 00:30+1h. One test fixture had to be retired: "caps the set" used fifteen identical 07:00 rows, which now all overlap. See DEVIATIONS.md. |
| 5 | **done** | main | All 3 tasks. 677 tests. Schema bumped to 2. `flow_enum` was NOT folded into `flow_fixed` — the fold cannot keep its `enum:<value>` variant key, which reuse is keyed on. LK-007 is CONFIRMED and deliberately not fixed: a `device` argument's value shape cannot be enumerated ahead of time and section 9's `time_exactly_day` is the standing example of what guessing costs. LK-030/031/047 and the time-card tie are all fixed. Fixture corpus is `reconstructed` — **needs a real hardware capture**, recipe in `test/fixtures/cards/README.md`. See DEVIATIONS.md. |
| 6 | **done** | main | All 6 tasks. 726 tests (712 statically counted — `plan-validation.test.ts` generates 13 from one `test()` call, which is what the README number tracks). `RuntimeRegistry`, `lib/support/migrations.ts` with required validators, `lib/validation/`, `lib/time/`, `lib/app-contract.ts`, `lib/homey-api-types.ts`, `lib/support/same.ts`. `no-explicit-any` is now an ERROR for `lib/**` with the seam files listed in `eslint.config.mjs`. S4's validators changed behaviour as predicted: four test fixtures were partial plans that a cast used to accept. See DEVIATIONS.md. |
| 7 | **done** | main | All 5 tasks. 737 tests. The settings page has no `innerHTML` and no `escapeHtml` at all; two form builders keep both, allowlisted with the argument written out in `webview-safety.test.ts`. `LIGHTKEEPER_DEVICE_ID` matches the legacy shape PERMANENTLY — a device id is baked into every Flow that device owns. LK-064 declined; `sync:views:check` runs in CI. See DEVIATIONS.md. |
| 8 | **done** | main | All 3 tasks. 771 tests. The circadian light is now the SIMPLE one (two ends, fixed shape); the point editor is a new `curve` driver, and a point may carry a palette colour. One registry serves both, so §12's one-timer property still holds. Schema 1→2 keeps the warmest and coolest points and drops the rest. The curve driver ships PLACEHOLDER artwork — `assets.test.ts`'s `PENDING_ARTWORK` is the definition of done. See DEVIATIONS.md. |
