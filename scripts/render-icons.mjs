 ...
 * empty circles on the store listing, and there was no way to have seen that
 * coming: the store draws an icon smaller than anywhere else does, and nothing
 * local drew them at that size. Read off homey.app's own stylesheet:
 *
 ...
 *
 * WHAT IT DOES NOT DIAGNOSE. A blank circle on a published listing is NOT
 * evidence that a drawing is too fine. 0.5.1 redrew four masters on that
 * reading and the circles stayed blank; the fault was a cache/CORS transient in
 * the browser, and a hard refresh brought every glyph back unchanged, at the
 * same 40 units. Platform section 10 has the three causes and how to tell them
 * apart — go there first, and come here once the question really is "does this
 * drawing read".
 *
...
the only size that decides whether a drawing reads: a 960 canvas rendered into 24px, where the
house 40-unit stroke is exactly 1.00px. The rightmost column is the same file at ten times that, so
a drawing fault can be told apart from a rasterising one. Nothing here passes or fails &mdash;
test/unit/assets.test.ts is what checks an icon, and a circle that is blank on the live listing is a
different question entirely (platform &sect;10).</p>
